export class PartitaScacchi {
  ...
  makeMove(uci) {
    ...
    if (!piece) throw new Error(`Nessun pezzo in ${from}`);

    if (this.activeColor === 'w' && !this.isWhite(piece)) throw new Error('Tocca al bianco');
    if (this.activeColor === 'b' && !this.isBlack(piece)) throw new Error('Tocca al nero');
